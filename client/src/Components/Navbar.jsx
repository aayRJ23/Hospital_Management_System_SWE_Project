      ...
      >
        ...
        {isAuthenticated ? (
          ...
          <div className="flex flex-col mt-10">
            ...
            <button
              className="w-full h-10 bg-blue-600 text-white rounded-lg font-semibold relative"
              onClick={() => setLoginOptionsOpen((prev) => !prev)}
            >
              LOGIN
              {loginOptionsOpen && (
                <div className=" bg-blue-600 absolute left-full top-0 flex flex-col rounded-xl w-48 mt-3 font-semibold items-center shadow-lg border border-gray-200 transform transition-transform duration-300 ease-in-out">
                  <Link
                    to={"/login"}
                    className="py-2 w-full text-center hover:bg-gray-100 hover:text-blue-600"
                  >
                    Patient
                  </Link>
                  <Link
