...
const LoginDoctor = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ✅ FIX: Was navigating to "/" — doctors should go to /doctor-home after login
  ...
  if (isAuthenticated) {
    // ✅ FIX: Redirect to doctor-home if already authenticated as doctor
    return <Navigate to={"/doctor-home"} />;
  }

  return (
    <motion.div
      className="flex flex-col lg:flex-row min-h-screen bg-gradient-to-r from-blue-500 to-indigo-500"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <div className="lg:w-1/2 flex flex-col justify-center items-center bg-white bg-opacity-30 backdrop-blur-lg p-10 rounded-br-full shadow-xl">
        ...
        <button
          className="w-44 mt-5 py-2 rounded-2xl font-semibold border-2 border-white text-white hover:bg-white hover:text-blue-500 transition duration-300"
          onClick={goToHome}
        >
          Home
        </button>
      ...
      <div className="lg:w-1/2 flex flex-col justify-center items-center bg-white bg-opacity-20 backdrop-blur-lg p-10 shadow-xl mt-10 lg:mt-0 lg:rounded-tl-full">
        ...
        <h4 className="text-xl text-white font-bold mb-5">Doctors Only</h4>
        <form
        ...
        >
          ...
          <motion.button
            className="w-40 bg-indigo-600 rounded-2xl h-10 text-white font-semibold shadow-lg hover:bg-indigo-700 transition duration-300"
            type="submit"
